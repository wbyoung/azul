...
module.exports = Mixin.create(/** @lends ModelQuery# */ {
   ...
   * {@link ModelQuery#__associatePrefetchResults}.
   *
   * @method
   * @private
   ...
   * completed.
   */
  _prefetchAssociation: BluebirdPromise.method(function(association, records, cache) {
  ...
  /**
   * Associates pre-fetch results via {@link BaseRelation#associatePrefetch}.
   ...
   * `associatePrefetch` method called. The objects passed to each
   * `associatePrefetch` call will begin with the set of objects that are given
  ...
  _associatePrefetchResults: function(association, records, cache) {
    ...
    this._relationForEach(association, function(assoc, rel, through, index) {
      ...
      if (associate) {
        relation.associatePrefetch(currentRecords,
