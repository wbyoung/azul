...
// TODO: docs & rename & move to a place where it can be used for `with` as well
var name = function(name, fn) {
  return function() {
    this._relationLookupFailureReason = name;
...
module.exports = Mixin.create(/** @lends ModelQuery# */ {
   ...
   * @see {@link Join#join}
   */
  join: name('join', function(/*association*/) {
  ...
  _joinAssociation: function(association) {
    ...
    this._relationForEach(association, function(assoc, relation) {
