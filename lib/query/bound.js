...
BoundQuery.reopen(/** @lends BoundQuery# */ {
  ...
  _create: function(model) {
    var _super = this._super.unbound;
    this._super(model.tableName);
    this._model = model;
    this._modelTable = model.tableName; // save in case query is unbound

    // bound queries need to re-initialize the select query when anything
    // changes that affects the generation of column names. this saved call to
    // super allows that to happen whenever necessary.
    this._recreate = function() {
      return _super.call(this, model.tableName);
    };
  },
   ...
   * @see {@link BoundCore#_take}
   */
  _take: function(orig) {
    // most of the duplication is handled in the core mixin
    this._super(orig);
  ...
  /**
   * Override of {@link Join#join}.
   *
   * When a join occurs, re-initialization of the select base class is required
  ...
  join: function(/*association*/) {
    var dup = this._super.apply(this, arguments)._dup();
    dup._recreate.call(dup);
    return dup; // see note in documentation above
  },
