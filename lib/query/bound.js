...
BoundQuery.reopen(/** @lends BoundQuery# */ {
  ...
  _create: function(model) {
    ...
    this._recreate = function() { // saved call to super for use in _reset
  ...
  /**
   * Bound queries need to be reset when anything changes that affects the
   * generation of column names.
   *
   * The typical case for this is when a join occurs because column names will
   * be transformed by {@link BoundTransform} from `select *` to
   * `select table.*`. The {@link BoundJoin} will therefore need to call
   * `_reset` on the resulting query to ensure that the columns are
   * processed again.
   *
   * @method
   * @protected
   * @return {ChainedQuery} The newly configured query.
   * @see {@link BoundCore#_reset}
