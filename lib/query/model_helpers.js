...
module.exports = Mixin.create(/** @lends ModelQuery# */ {
   ...
   */
  _relationForEach: function(association, fn, options, thisArg) {
    var opts = _.defaults({}, options, { source: false });
    // TODO: refactor
    ...
    path.forEach(function(name, index) {
      ...
      if (relation._options.through) { // TODO: REMOVE PRIVATE ACCESS
        ...
        if (!opts.source) {
          throughRelations = throughRelations.slice(1);
        }

        throughRelations.reverse().forEach(function(rel) {
          throughAssoc.push(rel._name);
          fn.call(thisArg, throughAssoc.join('.'), rel, relation, -1);
        });
