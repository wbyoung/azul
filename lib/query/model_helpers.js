...
module.exports = Mixin.create(/** @lends ModelQuery# */ {
   ...
   *
   * The _source_ of a through relation will actually cause two invocations of
   * the function to occur. The first will be with the fully expanded
   * association name. The second will be with the _through_ association name
   * used in the given association string. The first one will have the
   * `expanded` detail flag set, and the second one will not.
   *
   * @method
   * @private
  ...
  _relationForEach: function(association, fn, thisArg) {
    ...
    path.forEach(function(name, index) {
      ...
      if (relation._options.through) { // TODO: REMOVE PRIVATE ACCESS
        assoc.push(name);
        this._throughRelationForEach(assoc.join('.'), relation, fn, thisArg);
      }
      else {
        assoc.push(name);
  ...
  /**
   * Enumerate a through relation. This is a helper for
   * {@link ModelQuery#_relationForEach} and should not be used directly.
   *
   * Calls the callback with arguments compatible with
   * {@link ModelQuery#_relationForEach}.
   *
   ...
   *
   * @param {String} association Relation key path.
   * @param {Relation} relation The through relation to iterate.
   * @param {ModelQuery~RelationIterator} fn The callback function.
   * @param {Object} thisArg The value of `this` for the callback function.
   */
  _throughRelationForEach: function(association, relation, fn, thisArg) {
    ...
    var throughAssoc = assoc.slice(1);
