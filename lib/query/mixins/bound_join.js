...
module.exports = Mixin.create(/** @lends ModelJoin# */ {
   ...
   *
   * This also resets the resulting query to ensure that column names are
   * correct. For instance, `select *` will usually become `select table.*` on
   * bound queries once a join is performed. See {@link BoundQuery#_reset}
   * for details.
   *
   * @method
   * @public
  ...
  join: context('join', function(/*association*/) {
    var result = this._model && arguments.length === 1 ?
      this._joinAssociation(arguments[0]) :
      this._super.apply(this, arguments);
    return result._reset(); // see note in documentation above
  }),
