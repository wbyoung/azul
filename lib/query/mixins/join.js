...
/**
 * Check if an argument is a condition.
 *
 * @function Join~isCondition
 * @private
 * @param {?} arg The argument to check.
 * @return {Boolean}
 */
var isCondition = function(obj) {
  return !_.isString(obj) || obj.indexOf('=') !== -1;
};

/**
 * Check if an argument is an alias.
 *
 * @function Join~isAlais
 * @private
 * @param {?} arg The argument to check.
 * @return {Boolean}
 */
var isAlias = function(arg) {
  return !isType(arg) && !isCondition(arg);
};
...
module.exports = Mixin.create(/** @lends Join# */ {
   ...
   * @param {String} [alias] The alias name for the table.
   ...
   */
  join: function(/*table, [alias], [type], ...[condition]*/) {
    var dup = this._dup();
    var args = _.toArray(arguments);

    var table = args.shift();
    var alias = isAlias(args[0]) ? args.shift() : undefined;
