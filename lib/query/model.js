...
var ModelQuery = BoundQuery.extend(/** @lends ModelQuery# */ {
  ...
  _fieldTransformRelationsToTables: function(field) {
    ...
    var table = parts[1];
    var association = table;
    var relationData = association && this._joinedRelations[association];
    if (relationData) {
      parts[1] = relationData.alias;
    }
    return parts.reverse().join('.');
  },
  ...
  with: function(/*association...*/) {
    var dup = this._dup();
    var prototype = this._model.__class__.prototype;
    var prefetches = _.map(arguments, function(association) {
      var relation = prototype[association + 'Relation'];
      if (!relation) {
        throw new Error(util.format(
    ...
    dup._prefetches = dup._prefetches.concat(prefetches);
    return dup;
  },
  ...
  join: function(/*association*/) {
    ...
    if (arguments.length === 1) {
      ...
      var table = relation.relatedModelClass.tableName;
      var alias;
      if (table === this._model.tableName) {
        alias = relation._name;
        table = _.object([[table, alias]]);
      }
      var fk = [this._model.tableName, relation.foreignKey].join('.');
      var pk = [alias || table, relation.primaryKey].join('.');
      var condition = [fk, pk].join('=');

      dup = this._super(table, 'inner', condition);
      dup._joinedRelations[alias || association] = {
        alias: alias || table,
        relation: relation
      };
    }
