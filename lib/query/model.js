...
var ModelQuery = BoundQuery.extend(/** @lends ModelQuery# */ {
  ...
  _fieldTransformRelationsToTables: function(field) {
    ...
    if (relationData) {
      parts.splice(1, parts.length, relationData.alias);
    }
    return parts.reverse().join('.');
  },
  ...
  _joinAssociation: function(association) {
    return this._associationReduce(association, function(query, assoc) {
      ...
      return query._joinRelation(assoc, relation);
    }, this._dup(), this);
  },
   ...
   * @return {ChainedQuery} The newly configured query.
   */
  _joinRelation: function(name, relation) {
    ...
    var fk = [relation.modelClass.tableName, relation.foreignKey].join('.');
    var pk = [alias || table, relation.primaryKey].join('.');
    var condition = [fk, pk].join('=');


    var dup = this.join(table, 'inner', condition);
    dup._joinedRelations[alias || name] = {
      alias: alias || table,
      relation: relation
    };
    return dup;
