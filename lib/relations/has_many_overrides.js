...
module.exports = Mixin.create(/** @lends HasMany# */ {
   ...
   *   - Objects associated via the inverse relation w/ a dirty foreign key
   *
  ...
  reliants: function(relation) {
    return function() {
      ...
      // search for objects that were added by the inverse relation that are
      // still dirtied by the foreign key of this relation. these were not
      // added explicitly to the relation.
      var associated = data.associated;
      _.forEach(associated, function(related) {
        if (_.contains(related.dirtyAttributes, foreignKeyAttr)) {
          result.push(related);
        }
      });

      // search for added/removed objects that won't be fully saved by a bulk
      // foreign key update.
      var changed = data.add.concat(_.filter(data.remove, 'persisted'));
      _.forEach(changed, function(related) {
        var dirty = _.without(related.dirtyAttributes, foreignKeyAttr).length;
        var save = dirty || related.newRecord;
        if (save) {
          result.push(related);
        }
      });
