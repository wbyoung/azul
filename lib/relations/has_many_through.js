...
var throughOverride = function(fn) {
  return function() {
...
module.exports = Mixin.create(/** @lends HasMany# */ {
   ...
   */
  scopeObjectQuery: throughOverride(function(instance, query) {
  ...
  /**
   * Handle association of through relations.
   *
   * @method
   * @protected
   * @see {@link HasMany#associateObjects}
   */
  associateObjects: throughOverride(function(instance, objects, options) {
    ...
    var actions = objects.map(function(object) {
      ...
      // TODO: don't have the source relation perform a full association. it
      // should not cache the association information. either split association
      // of attributes into its own separate method that can be called
      // individually here. for instance, `associateValues`. or add another
      // option flag here which would allow disabling of the adding to caches.
      // var sourceOptions = _.extend({}, options, { cache: false });
      // sourceRelation.associate(joinModel, instance, sourceOptions);
      // sourceRelation.associate(joinModel, object, sourceOptions);
      return Actionable.create(function() {
        ...
        .then(function() {
          var insert = {};
          insert[throughRelation.foreignKey] = instance.pk;
          insert[sourceRelation.foreignKey] = object.pk;
          var query = sourceRelation._modelClass.objects.insert(insert);
  ...
  /**
   * Handle disassociation of through relations.
   *
   * @method
   * @protected
   * @see {@link HasMany#disassociateObjects}
   */
  disassociateObjects: throughOverride(function(instance, objects, options) {
    var opts = _.defaults({}, options, { follow: true, attrs: true });
    var throughRelations = this._throughRelations();
    var sourceRelation = throughRelations[0];
    var throughRelation = throughRelations[1];
