...
module.exports = Mixin.create(/** @lends HasMany# */ {
  ...
  init: function(name, attributeDetails) {
    ...
    if (this._autoJoin()) {
      this._options.through = inflection.pluralize(this._options.join);
    }
    if (this._options.through) {
    ...
    if (this._autoJoin()) {
      var belongsToName = this._options.source || this._name;
      var belongsTo = require('./belongs_to').attr();
      var belongsToAttrs = _.object([[belongsToName, belongsTo()]]);
