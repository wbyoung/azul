...
module.exports = Mixin.create(/** @lends HasMany# */ {
  ...
  configure: override(function() {
    ...
    // a join table or a shortcut to a source defined on another model. either
    // way, defining two `hasMany` relations every time you want to define
    // either of these configurations would be tedious. we therefore assume
    // that if `through` is specified, but there is no relation on the current
    // model that matches, that we should add an implicit `hasMany` to
    // represent the through relation.
    var throughRelation = modelClass[through + 'Relation'];
    if (!throughRelation && !implicit) {
      var hasManyModel = this._joinModel();
      var hasManyAttr = db.hasMany(hasManyModel, { implicit: true });
    ...
    // the implicit `belongsTo` relationship.
    ...
    this.expand(); // pre-verify that expansion works
    this._super();
  }),
  ...
  _isToMany: override(function() {
    ...
    if (!sourceRelation && !this._options.source) {
  ...
  _expansionDetails: override(function() {
    ...
    while (relation) {
      ...
      var throughRelation = through && modelClass[through + 'Relation'];

      // we collect all non-through relations
      if (!through) { relations.push(relation); }
      ...
      if (!through && relatedModel === targetModel) { relation = null; }
        ...
        // we're jumping to a new model class now
        var jumpToModel = relation._relatedModel;
        var search = _.uniq([
          source,
        ...
        var jumpRelation = _(search)
          .map(function(name) { return name + 'Relation'; })
        ...
        if (jumpRelation) {
          ...
          details.error = new Error(util.format(
            ...
            modelName, relation._name, this._modelClass.__name__, this._name));
          relation = null;
        }
      }
    }
