...
module.exports = Mixin.create(/** @lends HasMany# */ {
  ...
  scopeObjectQuery: function(query, ids) {
    ...
    joinableRelations.forEach(function(relation, index) {
      ...
      primaryKey = [joinName, sourceRelation.foreignKey].join('.');
    ...
    // TODO: this does not return a model query (which would be expected)
    return query.where(where).all(['*', foreignKey]);
  },
