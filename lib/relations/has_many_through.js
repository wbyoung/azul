...
// TODO: this is duplicated from `has_many.js`
...
module.exports = Mixin.create(/** @lends HasMany# */ {
  ...
  // TODO: docs
  // TODO: is this method even right
   ...
   */
  executeAdd: override(invalidatesQuery(function(instance, objects) {
   ...
   */
  executeRemove: override(invalidatesQuery(function(instance, objects) {
    if (!objects.length) { return; }
   ...
   * @see {@link HasMany#executeRemove}
   */
  executeClear: override(invalidatesQuery(function(instance) {
    var throughRelations = this._throughRelations();
    var sourceRelation = throughRelations[0];
    var throughRelation = throughRelations[1];
  ...
  /**
   * Handle pre-fetch for through relations returning an empty set of results.
   *
   * Pre-fetching of through relations is handled by
   * {@link ModelQuery#_prefetch} when through relations are expanded.
   *
   * @method
   * @protected
   * @see {@link HasMany#associatePrefetchResults}
   */
  // TODO: this method will never be called. test it or delete it.
  prefetch: override(BluebirdPromise.method(function(instances) {
    return {};
  })),
