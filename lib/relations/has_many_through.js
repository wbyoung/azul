...
module.exports = Mixin.create(/** @lends HasMany# */ {
  ...
  configure: override(function() {
    ...
    if (canBeToMany && !implicit) {
      ...
      var sourceRelation = joinModel['_' + source + 'Relation'];
      if (!sourceRelation) {
  ...
  /**
   * Override of {@link HasMany#_inverse} that ensures that many-to-many
  ...
  _joinTable: override(function() {
    var result;
    if (this._options.through && this._isToMany) {
      result = this._joinModel().tableName;
    }
    else if (this._options.through) {
      result = this._options.through;
    }
    else {
      result = this._super();
    }
  ...
  _expansionName: override(function() {
    return this._super() + '_through_' + _.snakeCase(this._options.through);
  }),
