...
HasMany.reopen(/** @lends HasMany# */ {
  ...
  collection: function(instance) {
    // TODO: we want to actually have the cache built for the through
    // association, so none of this code should be here. instead, changes will
    // need to be made in `ModelQuery#with` and the relation prefetches.
    if (this._options.through) {
      ...
      return models.map(function(model) { // TODO: move return
        return soruceRelation.item(model);
      });
    }
