...
HasMany.reopen(/** @lends HasMany# */ {
  ...
  collection: function(instance) {
    ...
    //   // TODO: move this so it's not accessing semi-private method
    //   var throughRelations = this._throughRelations();
    //   if (throughRelations.length !== 2) {
    //     throw new Error('Cannot get collection on this type of through ' +
    //       'relation');
    //   }
    //   var throughRelation = throughRelations[1];
    //   var soruceRelation = throughRelations[0];

    //   var models = throughRelation._getCollectionCache(instance);
    //   if (!models) {
    //     throw new Error(util.format('The through relation "%s" has not yet been ' +
    //       'loaded.', this._name));
    //   }
    //   return models.map(function(model) { // TODO: move return
    //     var result = soruceRelation._related(model);
    //     if (!result) {
    //       throw new Error(util.format('The through relation "%s" has not ' +
    //         'been fully loaded.', this._name));
    //     }
    //     return result;
    //   });
    // }
  ...
  objectsQuery: function(instance) {
    ...
    if (!instance[cacheName]) {
      ...
      var pk = instance.getAttribute(this.primaryKey);
      instance[cacheName] = this
        .scopeObjectQuery(this._relatedModel.objects, [pk])
   ...
   * @param {Array.<?>} ids The ids that this query should fetch.
   * @return {ChainedQuery} query The newly configured query.
   */
  _scopeObjectQuery: function(query, ids) {
    var queryKey = this.foreignKey;
    if (ids.length === 1) { ids = ids[0]; }
    else { queryKey += '[in]'; }
    var where = _.object([[queryKey, ids]]);

    if (ids.length > 1) {
      query = query.limit(ids.length);
    }
