...
var HasManyRelationQuery = BoundQuery.extend({
  ...
  add: function() {
    ...
    var additions = _.flatten(Array.prototype.slice.call(arguments));
    var ids = _.map(additions, 'id');
    additions.forEach(function(addition) {
      addition[foreignKeyProperty] = this._model.id;
      addition[inverse] = this._model;
    ...
    // TODO: really bad to access private _where and clear it out, come back to
    // this and figure out how to clear the where or not pre-specify the
    // foreign-key where part.
    var query = this.clone();
    query._where = undefined;
    return query.where(conditions).update(updates);
  },
