...
BelongsTo.reopen(/** @lends BelongsTo# */ {
   ...
   * `article.getAuthor`.
   *
   * The naming conventions are set forth in {@link BelongsTo#overrides}.
   *
   * @method
   * @protected
  ...
  set: invalidatesQuery(function(instance, value) {
    ...
    // always ensure cached value is set in case neither associate/disassociate
    // ends up being called.
    this._setRelated(instance, value);
  ...
  overrides: function() {
    ...
    if (!this._options.implicit) {
      ...
      this.addHelper('create<Singular>', 'create');
      this.addHelper('fetch<Singular>', 'fetch');

      this.addOverride('<foreignKey>', _.extend(function(/*relationKey*/) {
      ...
      this.overrideMethod('_presave', 'presave');
