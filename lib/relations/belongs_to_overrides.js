...
module.exports = Mixin.create(/** @lends BelongsTo# */ {
   ...
   *
   * This simply calls super, but its inclusion ensures that the relation will
   * be configured during the initialization of any object.
   *
  ...
  dependents: function(relation) {
    return function() {
      ...
      // we only depend on the related object being saved if it's a new record
      // or the primary key attribute is dirty. otherwise, the update can
      // proceed without saving the related object (the save is not dependent
      // on the update of the related object).
      var dependent = false;
      if (related) {
        var newRecord = related.newRecord;
        var dirtyAttributes = related.dirtyAttributes;
        var primaryKeyAttr = relation.primaryKeyAttr;
        var primaryKeyIsDirty = _.contains(dirtyAttributes, primaryKeyAttr);
        dependent = newRecord || primaryKeyIsDirty;
      }

      if (dependent) { result.push(related); }
  ...
  _updateForeignKey: function(instance) {
    ...
    var relatedIsLoaded = (related !== undefined); // ensure valid cache

    var current = instance.getAttribute(foreignKeyAttr);
    var updated = _.get(related, primaryKey);
    var changed = relatedIsLoaded && (current !== updated);
    if (changed) {
