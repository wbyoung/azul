...
module.exports = Mixin.create(/** @lends Phrasing# */ {
  ...
  alterTable: function(data) {
    ...
    var renamed = data.renamed.map(this.renameColumn, this);
    var delimited = this._grammar.delimit([].concat(dropped, added, renamed));
    var joined = this._grammar.join(delimited);
    var fragments = ['ALTER TABLE '];
    fragments.push(quoteField(data.name), ' ');
    fragments = fragments.concat(joined);
    var statement = Statement.create(this._grammar.join(fragments));
    return alterable ? statement : undefined;
  },
