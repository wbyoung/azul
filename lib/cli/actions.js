...
module.exports.init = function(options) {
  ...
  else if (!_.contains(['pg', 'mysql', 'sqlite3'], options.database)) {
    process.stderr.write(chalk.red(
      util.format('Invalid database: %s\n', options.database)));
    process.exit(1);
  }
  else {
    var sourceName = 'azulfile-' + options.database + '.js';
    var source = path.join(__dirname, '../templates', sourceName);
    fs.writeFileSync('./azulfile.js', fs.readFileSync(source));
