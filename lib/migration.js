...
var Migration = Class.extend(/** @lends Migration# */ {
  ...
  /**
   * @function Migration~TransactionFunction
   * @param {EntryQuery} query A query/transaction object.
   * @param {Schema} schema A schema query in the transaction.
   * @return {Promise}
   */
  ...
  _transaction: function(fn) {
    var transaction = this._query.transaction();
    var begin = transaction.begin();
    var query = this._query.transaction(transaction);
    var schema = query.schema().transaction(transaction);
    return begin.execute().bind({})
    .then(fn.bind(this, query, schema))
   ...
   * @return {Promise}
   */
  _execute: BluebirdPromise.method(function(fn, schema, query) {
    var captured = [];
    var creators = [];
    var executed = false;
    var reused;
    var observe = function(q) {
      creators.push(this);
      captured.push(q);
      q.on('spawn', observe);
      q.on('dup', observe);
    ...
    if (!promise && executed) {
      ...
      result = BluebirdPromise.each(captured, function() {});
    }
  ...
  migrate: function() {
    var self = this;
    return this._transaction(function(query, schema) {
      return this._loadPendingMigrations()
      .each(function(module) {
        return self._execute(module.up, schema, query);
      })
