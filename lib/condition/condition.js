...
Condition.reopen(/** @lends Condition# */ {
  ...
  build: function(grammar, translator) {
    var transform = function(item) {
      var method = '_transform' + _.str.capitalize(item.type) + 'Node';
      return this[method] ? this[method](item, translator) : item;
    };
    var reduce = function(item, children) {
      var method = '_reduce' + _.str.capitalize(item.type) + 'Node';
...
Condition.reopen(/** @lends Condition# */ {
  ...
  /**
   * @function Condition~TransformFunction
   * @param {String} field The field name to transform.
   * @return {String} The transformed name.
   */

  /**
   * Transform the names of fields used within the predicate. This can be used,
   * for instance, to prepend a table name to each of the fields within the
   * condition.
   *
   * @method
   * @protected
   * @param {Condition~TransformFunction} fn The transformation function.
   * @return {Condition} A new condition that has the transformation applied.
   */
  transformFields: function(fn) {
    var transform = function(node) {
      if (isLeafNodeWithFieldString(node)) {
        var value = fn(node.attrs.value.toString());
        var replacement = FieldString.create(value);
        node = Node.create('leaf', [], { value: replacement });
      }
      return node;
    };
  ...
  /**
   * @function Condition~ReduceFunction
  ...
  /**
   * Reduce the names of fields used within the predicate.
   *
   * @method
   * @protected
   * @param {Condition~ReduceFunction} fn The reduce function.
   */
