...
Condition.reopen(/** @lends Condition# */ {
  ...
  _build: function(item, options) {
    ...
    if (item.type === 'tree') {
      ...
      // value (with the type check to ensure it's a value)?
      var lhs = item.lhs;
      var rhs = item.rhs;
      if (rhs.type !== 'value' && rhs.type !== 'field') {
        throw new Error('Right hand side can only be field or value');
      }
      if (rhs.type === 'value') {
        var details = _.clone(rhs); // clone to allow alteration
        var predicate = item.predicate;
        var transformed = grammar.transformForExpression(predicate, details);
        rhs = Node.create(transformed);
      }
      result = grammar.expression(
        this._build(lhs, opts),
        translator.predicate(item.predicate),
        this._build(rhs, opts));
    }
