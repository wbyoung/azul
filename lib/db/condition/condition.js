...
Condition.prototype._initialize = function(parts) {
  parts.forEach(function(part) {
    if (Array.isArray(part)) {
      ...
      var pair = part.split('=');
      var key = pair[0];
      var value = new FieldString(pair[1]);
      var object = {};
      object[key] = value;
...
Condition.prototype._build = function(grammar, item) {
  ...
  if (item.type === 'tree') {
    ...
    // TODO: this is the only place that predicate conversion is going on. this
    // is related to the TODO in the grammar file. possible ways of moving the
    // predicate mapping would be to put it into the adapter/phrasing. that
    // would then require either more arguments or a change of arguments sent
    // to build the condition. the adapter could be sent to build conditions
    // which would allow the condition to look up anything it needed to, but
    // may be a little bit too much coupling. another option would be to just
    // make another translation class. name it something like `Translation` or
    // `Mapping` to handle the translation of simple words.
    // perhaps the whole expression part belongs in the `Phrasing` class, though
    // since the order of the pieces can matter.
