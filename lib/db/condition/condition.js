...
Condition.reopen(/** @lends Condition# */ {
  ...
  _build: function(item, options) {
    ...
    var valueTransform = opts.valueTransform;

    if (item.type === 'tree') {
      result = grammar.join(this._build(item.tree, opts));
    }
      ...
      // TODO: should the value transform only apply to the right hand side or
      // value (type check to ensure it's a value)? if type checking the value,
      // would the transformer function even be needed?
      // TODO: would it be easier or more understandable to create a new tree
      // node for the right hand side that includes a value transformation?
      var lhs = this._build(item.lhs, opts);
      var rhs = this._build(item.rhs, _.extend(opts, {
        valueTransform: grammar.predicateValueTransformer(item.predicate)
      }));
      var predicateFormat = translator.predicate(item.predicate);
      result = grammar.expression(lhs, predicateFormat, rhs);
    }
    else if (item.type === 'field') {
      result = grammar.field(item.field);
    }
    else if (item.type === 'value') {
      result = grammar.value(valueTransform ?
        valueTransform(item.value) : item.value);
    }
