  ...
  parts.forEach(function(part) {
    if (part instanceof Operation.__class__) { result.push(part); } // ignore
      ...
      Object.keys(part).forEach(function(key) {
        ...
        var lhs = Node.create({ type: 'field', field: details.key });
        var rhs = part[key] instanceof FieldString.__class__ ?
          Node.create({ type: 'field', field: part[key].toString() }) :
          Node.create({ type: 'value', value: part[key] });
 ...
 * @param {Translator} translator The translator to use to construct the
 * condition.
 * @return {Fragment} The fragment that results from building the condition.
 */
Condition.prototype._build = function(item, grammar, translator) {
  var result;

  if (item.type === 'tree') {
    result = grammar.joinFragments(this._build(item.tree, grammar, translator));
  }
  else if (item.type === 'binaryOperation') {
    result = grammar.operation(
      this._build(item.lhs, grammar, translator), item.operator,
      this._build(item.rhs, grammar, translator));
  }
  else if (item.type === 'unaryOperation') {
    result = grammar.unary(item.operator,
      this._build(item.operand, grammar, translator));
  }
  else if (item.type === 'group') {
    result = grammar.group(
      this._build(item.group, grammar, translator));
  }
  else if (item.type === 'expression') {
    var predicateFormat = translator.predicate(item.predicate);
    result = grammar.expression(
      this._build(item.lhs, grammar, translator), predicateFormat,
      this._build(item.rhs, grammar, translator));
  }
 ...
 * @method
 */
Condition.prototype.build = function(grammar, translator) {
  return this._build(this._tree, grammar, translator);
};
...
Condition.FieldString = FieldString;
Condition.f = FieldString.f;
Condition.w = _.extend(Condition.create.bind(Condition), Condition);

Condition.reopenClass({ __name__: 'Condition' });
