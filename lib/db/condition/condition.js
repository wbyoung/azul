...
Condition.reopen(/** @lends Condition# */ {
  ...
  _build: function(item, options) {
    ...
    if (item.type === 'tree') {
      ...
      // sent to to the translator's predicate function for possible
      // alteration. afterwards, they're converted back into leaf nodes before
      ...
      result = grammar.expression(format, finalArgs);
    }
      ...
      if (value instanceof LiteralString.__class__) {
        ...
        result = grammar.field(value.toString());
      }
      else {
        result = grammar.value(value);
      }
    }
