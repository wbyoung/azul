...
var RawQuery = Class.extend(/** @lends RawQuery# */{
   ...
   */
  execute: function() {
    // TODO: this needs to send transaction arguments/options off to the
    // adapter. it also needs to take an options argument if the current
    // transaction code remains the same.
    if (!this._executed) {
      var statement = this.sql();
      var self = this;
      this._executed = true;
      return self._adapter.execute(statement.sql, statement.arguments)
...
RawQuery.reopen(Transaction); // transaction methods override raw query methods
