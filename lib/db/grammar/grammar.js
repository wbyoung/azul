...
var Grammar = Class.extend(/** @lends Grammar# */{
   ...
   */
  transformForExpression: function(predicate, details) {
    var value = details.value;
    if (predicate.match(/^isnull$/i)) {
      // TODO: this should probably be something more like 'literal' to
      // differentiate from a field.
      details.type = 'field';
      details.field = value ? 'NULL' : 'NOT NULL';
      delete details.value;
    }
    else if (predicate.match(/^i?contains$/i)) {
      details.value = util.format('%%%s%%', value);
    }
    else if (predicate.match(/^i?startswith$/i)) {
      details.value = util.format('%s%%', value);
    }
    else if (predicate.match(/^i?endswith$/i)) {
      details.value = util.format('%%%s', value);
    }
    else if (predicate.match(/^i?regex$/i) && value instanceof RegExp) {
