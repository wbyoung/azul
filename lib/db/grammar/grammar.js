...
var Grammar = Class.extend(/** @lends Grammar# */{
  ...
  predicateValueTransformer: function(predicate) {
    var result;
    if (predicate === 'contains' || predicate === 'icontains') {
      result = function(value) { return util.format('%%%s%%', value); };
    }
    else if (predicate === 'startswith' || predicate === 'istartswith') {
      result = function(value) { return util.format('%s%%', value); };
    }
    else if (predicate === 'endswith' || predicate === 'iendswith') {
      result = function(value) { return util.format('%%%s', value); };
    }
    else if (predicate === 'regex' || predicate === 'iregex') {
      // convert regular expressions to strings (and strip out any js regex
      // options)
      result = function(value) {
        return value instanceof RegExp ?
          value.toString().replace(/^\//, '').replace(/\/[gimy]*$/, '') :
          value;
      }
    }
    return result;
