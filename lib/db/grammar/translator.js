...
var Translator = Class.extend(/** @lends Translator# */{
  ...
  _expression: function(predicate, details) {
    ...
    if (predicate.match(/^i?(?:contains|startswith|endswith)$/)) {
      // TODO: it'd be good to do that in a unicode-safe way if possible. this
      // does not feel like the suitable place for that. a better place is
      // probably in the grammar class as it's mostly related to characters &
      // symbols.
      value = value
        .replace('\\', '\\\\')
        .replace('%', '\\%')
