...
Translator.reopen(/** @lends Translator# */{
  ...
  /**
   * Alias of {@link Translator#serial}, do not override.
   * @private
   * @see {@link Translator#type}
   */
  typeForAuto: function() {
    return this.typeForSerial.apply(this, arguments);
  },
  ...
  typeForDecimal: function(options) {
    var opts = options || {};
    var digits = opts.precision; // total number of digits
    var scale = opts.scale; // total number of decimal places
    var result = 'numeric';
    if (digits !== undefined && scale !== undefined) {
      result = util.format('numeric(%d, %d)', digits, scale);
    }
    else if (digits !== undefined) {
      result = util.format('numeric(%d)', digits);
    }
    else {
