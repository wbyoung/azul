 ...
 * @return {Translator~PredicateTransformer} The transformer function.
 */
var wrapValue = function(/*fn...*/) {
  var fns = Array.prototype.slice.call(arguments);
  var fn = _.compose.apply(null, fns.reverse());
  return function(pred) {
    pred.args = pred.args.slice(0);
    pred.args[1] = fn(pred.args[1]);
    return pred;
  };
};
...
/**
 * This is a {@link Translator~PredicateTransformer} that will expand the
 * _value_ argument, the argument at index _1_ in the list of arguments, which
 * must be an array. This allows format strings to easily specify multiple
 * placeholders and accept array values to fill those (i.e. between).
 *
 * @since 1.0
 * @public
 * @method Translator.expandArgs
 */
var expandArgs = function(pred) {
  pred.args = pred.args.slice(0);
  pred.args = pred.args.concat(pred.args.pop());
  return pred;
};
...
/**
 * This is a {@link Translator~PredicateTransformer} that will expand the
 * format string and the _value_ argument (see {@link Translator.expandArgs}).
 * It will replace the first occurrence of `%@` in the format string with the
 * comma separated `%s` strings to match the number of items in the _value_
 * array. This allows format strings to expand based on value arguments (i.e.
 * to support `in` predicates).
 *
 * @since 1.0
 * @public
 * @method Translator.expandAll
 */
var expandAll = function(pred) {
  var args = pred.args;
  var length = args[1].length;
  var placeholders = _.times(length, function() { return '%s'; });
  pred.format = pred.format.replace('%@', placeholders.join(', '));
  return expandArgs(pred);
};
Translator.reopenClass({ expandAll: expandAll });
...
Translator.reopen(/** @lends Translator# */{
  ...
  predicate: function(predicate, args) {
    ...
    // TODO: is there a better way to specify transformers? perhaps make them
    // their own classes? right now, this feels really clunky. also, it feels
    // weird that the format & args are passed along to these these functions.
    // it's required, but still feels odd. the syntax of declaring each
    // predicate is pretty good, though. perhaps altering that syntax to be
    // something like this could help split things up and/or clarify:
    //    p('year').regex(/year/i).format('YEAR(%s) = %s')
    //    p('in').regex(/in/i).format('%s IN (%s)').expandFormat().expandArgs()
    //    p('between').regex(/.*/i).format('%s BETWEEN %s AND %s').expandArgs()
    // the encapsulation of this predicate definition into its own class could
    // certainly clear things up.
    var result = { format: match.format, args: args };
    return match.transformer ? match.transformer(result) : result;
  },
   ...
   * @property {Translator~PredicateTransformer} transformer The transformation
   * function.
   */
  ...
  /**
   * @callback Translator~PredicateTransformer
   * @param {{ format: String, args: Array }} pred The format string & array of
   * arguments to transform.
   * @return {{ format: String, args: Array }} A new format string & array of
   ...
   * @param {Translator~PredicateTransformer} transformer The format string &
   * argument transformer for the resulting predicate.
   */
   ...
   *
   * While possible to omit the {@link Translator~PredicateTransformer}
