...
var Phrasing = Class.extend(/** @lends Phrasing# */{
   ...
   */
  // TODO: implement
  // TODO: this is just placeholder code that may be useful for implementing
  // this function. it's been pulled out of the insert from above & the map
  // of the quoteValue.
  // var value = 'probably something being mapped through';
  // var quotedValue = value instanceof FieldString.__class__ ?
  //   quoteField(value) :
  //   quoteValue(value);
  // TODO: really it would be better to pass off the value object to the
  // grammar and allow that to handle fields/literals/values appropriately.
  // that would simplify Condition#_buildLeaf as well.
  ...
  createTable: function(data) {
    ...
    var columns = data.columns.map(function(column) {
      ...
      return util.format('%s %s', quoteField(column.name), type);
    }, this);
    ...
    fragments.push(util.format('%s ', quoteField(data.name)));
