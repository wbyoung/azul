...
var Phrasing = Class.extend(/** @lends Phrasing# */{
  ...
  select: function(data) {
    ...
    var fragments = [Fragment.create(
      util.format('select %s from %s',
        quotedColumns.join(', '),
        tables.join(', ')))];
  ...
  createTable: function(data) {
    var prefix = util.format('create table %s ', data.name);
    ...
    var joinedColumns = this._grammar.joinFragments(columns);
    var groupedColumns = this._grammar.group(joinedColumns);
    var fragments = [Fragment.create(prefix)].concat(groupedColumns);
    return Statement.create(this._grammar.joinFragments(fragments));
  }
});
