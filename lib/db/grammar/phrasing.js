...
var Phrasing = Class.extend(/** @lends Phrasing# */{
  ...
  select: function(data) {
    ...
    return Statement.create(this._grammar.joinFragments(fragments));
  },
  ...
  insert: function(data) {
    ...
    var split = _(data.values).pairs().zip().value();
    var columns = split[0].map(quoteField);
    var values = this._grammar.delimit(split[1].map(function(value) {
      return value instanceof FieldString.__class__ ?
        quoteField(value) :
        quoteValue(value);
    }));
    columns = this._grammar.group(this._grammar.joinFragments(columns));
    values = this._grammar.group(this._grammar.joinFragments(values));
  ...
  createTable: function(data) {
    ...
    var joinedColumns = this._grammar.joinFragments(delimitedColumns);
