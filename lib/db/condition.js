...
/**
 * Initialization method for extracting expressions.
 *
 * Documentation forthcoming.
 */
Condition.prototype._extractExpressions = function(parts) {
  var result = [];

  parts.forEach(function(part) {
    if (part instanceof Operation) { result.push(part); } // ignore
    else if (part instanceof Condition) {
      result.push({ type: 'group',  group: part._tree });
    }
    else {
...
/**
 * Initialization method for reducing unary parts.
 *
 * Documentation forthcoming.
 */
Condition.prototype._reduceUnary = function(parts) {
  ...
  parts.forEach(function(part) {
    if (part instanceof UnaryOperation) {
      ...
      while (unary.length) {
        part = { type: 'unaryOperation', operator: unary.pop(), operand: part };
      }
      result.push(part);
    }
  });
...
/**
 * Initialization method for reducing binary parts.
 *
 * Documentation forthcoming.
 */
...
Condition.prototype._construct = function() {
  ...
  parts = this._extractExpressions(parts);
