...
Condition.prototype._construct = function() {
  ...
  var prepend = function(expression) {
    tree = tree ? {
  ...
  this._parts.slice().reverse().forEach(function(part) {
    if (part instanceof BinaryOperation) { operator = part.name; }
    else if (part instanceof UnaryOperation) {
      tree = { type: 'unaryOperation', operator: 'not', operand: tree };
    }
    else if (part instanceof Condition) {
      prepend({ type: 'group',  group: part._tree });
    }
    else {
