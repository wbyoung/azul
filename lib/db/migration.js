...
var Migration = Class.extend(/** @lends Migration# */{
  ...
  _readPendingMigrations: function() {
    ...
    .spread(function(migrations, executedMigrations) {
      return _.difference(migrations, executedMigrations);
    });
  },
  ...
  _readExecutedMigrations: function() {
    ...
    .call('sort'); // TODO: could change the above query to order by name instead
  },
  ...
  _loadPendingMigrations: function() {
    return this._readPendingMigrations().bind(this).map(function(file) {
      return require(path.join(this._dir, file));
    });
  },
  ...
  migrate: function() {
    ...
    .then(function(migrations) {
      // TODO: insert into migrations table
      // return this._query.insert('azul_migrations').values([{
      //   name: 'name',
      //   batch: 1
      // }]);
    });
