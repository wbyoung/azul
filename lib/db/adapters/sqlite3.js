...
SQLite3Adapter.reopenClass(/** @lends SQLite3Adapter */ {
  ...
  Translator: Adapter.Translator.extend({
    predicates: function(p) {
      ...
      p('iexact').using(likeFormat);
      p('contains').using(likeFormat).value('like', 'contains');
      p('icontains').using(likeFormat).value('like', 'contains');
      p('startsWith').using(likeFormat).value('like', 'startsWith');
      p('istartsWith').using(likeFormat).value('like', 'startsWith');
      p('endsWith').using(likeFormat).value('like', 'endsWith');
      p('iendsWith').using(likeFormat).value('like', 'endsWith');
