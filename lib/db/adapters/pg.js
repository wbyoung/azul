...
PGAdapter.reopenClass(/** @lends PGAdapter */{
  ...
  Translator: Adapter.Translator.extend({
    ...
    _predicaetesForPGDates: function(p) {
      p('year', 'EXTRACT(\'year\' FROM %s) = %s');
      p('month', 'EXTRACT(\'month\' FROM %s) = %s');
      p('day', 'EXTRACT(\'day\' FROM %s) = %s');
      p('weekday', 'EXTRACT(\'dow\' FROM %s) = %s',
        wrapValue(date.parseWeekdayToInt));
      p('hour', 'EXTRACT(\'hour\' FROM %s) = %s');
      p('minute', 'EXTRACT(\'minute\' FROM %s) = %s');
      p('second', 'EXTRACT(\'second\' FROM %s) = %s');
    },
