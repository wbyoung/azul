...
Adapter.reopen(/** @lends Adapter# */ {
  ...
  /**
   * Create connection details via the `connect`. This is triggered by the
  ...
  _poolCreateConnection: function(callback) {
    ...
    this.connect().then(success, callback);
  },

  /**
   * Destroy connection details via `disconnect`. This is triggered by the
...
Adapter.defineAccessor('pool', function() {
  return this._pool = this._pool || BluebirdPromise.promisifyAll(pool.Pool({
    name: 'connection',
