...
Adapter.defineAccessor('pool', function() {
  ...
  var destroy = function(obj) {
    ...
    // from the `connect` method. so here, it gets passed along to `disconnect`
    // so the disconnect gets the same object that `connect` returned.
    self.disconnect(obj);
  };
