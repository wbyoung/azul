  ...
  invoke: function(name, reopen/*, prototype*/) {
    ...
    methods[relation] = property(function() {
      if (!this[relationPrivate]) {
        var modelName = this.__identity__.__name__;
        var modelNameUnderscored = _.str.underscored(modelName);
        var opts = _.defaults({}, options, { primaryKey: 'id' });
        opts.inverse = opts.inverse ||
          _.str.camelize(modelNameUnderscored);
        opts.foreignKey = opts.foreignKey ||
          _.str.underscored(opts.inverse + '_id');

        var where = _.object([[opts.foreignKey, this[opts.primaryKey]]]);
        // TODO: remove use of private method
        this[relationPrivate] =
          relatedModel.objects.where(where)._spawn(HasManyRelationQuery,
            [this, relatedModel, opts]);
      }
