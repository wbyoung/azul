...
var Model = Class.extend(/** @lends Model# */ {
  ...
  save: function(options) {
    ...
    var query;
    if (!this.persisted || this.dirty || opts.force) {
      query = this.__identity__.objects.insert(this.attrs).returning(pk)
      .then(function(result) {
        // TODO: while this works, the result is still being transformed, and
        // a new model object is being created just to get the primary key
        // back. this could be really confusing in the future, so it'd be good
        // to change the way that the transformations are set up so that they
        // don't apply to inserts.
        self.id = result[0][pk];
      });
    }
    return BluebirdPromise.resolve(query);
  },
...
Model.reopenClass(/** @lends Model */{
   ...
   * @method
   */
  createWithRawAttrs: function(attrs) {
    // TODO: rename, and change the value of persisted to true
