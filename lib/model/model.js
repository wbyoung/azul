...
// duplicate data being sent back to JS. having joins performed automatically
// based on the key in a `where` will require introspection of the condition &
 ...
 *     // -> select * from "users" where "user"."id" in (?, ?, ?) limit 3;
 *     // !> [3,5,8]
 *
 *     Article.objects.where({ 'blog.title[contains]': 'Azul' }).fetch().then('...');
 *     // -> select * from "articles" left join "blogs" on "articles"."blog_id" = "blogs"."id"
...
var Model = Class.extend(/** @lends Model# */ {
   ...
   * will be updated with the resulting value.
   *
   * @method
   * @private
   * @return {ModelQuery} The query object.
  ...
  save: BluebirdPromise.method(function(options) {
    ...
    if (this.newRecord) { query = this._insertQuery(); }
    else if (this._deleteExecuted) {} // do nothing for deleted objects
    else if (this._deletePending) { query = this._deleteQuery(); }
    else if (this.dirty || opts.force) { query = this._updateQuery(true); }
  ...
  /**
   * Whether this model is a new record, that is it has not yet been saved to
   * the database. Objects that are new records can still be
   * {@link Model#dirty}.
   *
   * @public
   * @type {Boolean}
...
Model.reopenClass(/** @lends Model */ {
   ...
   */
  load: function(attrs) {
    return _.extend(this.create({ _newRecord: false }), {
      _attrs: attrs
    });
  }
