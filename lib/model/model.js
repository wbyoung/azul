...
var Model = Class.extend(/** @lends Model# */ {
   ...
   * reading individual attributes rather than using {@link Model#dbattrs}.
   *
   * @method
   * @protected
   */
  getAttribute: function(attr) {
    return this.dbattrs[attr];
  },

  /**
   * Set an attribute by the database column name. This is the only acceptable
   * way to set attributes in `dbattrs`. Do set them directly.
   *
   * @method
   * @protected
   */
  ...
  _whereQuery: function(baseQuery) {
    return baseQuery.where(_.pick(this.dbattrs, this.pkAttr));
  },
  ...
  _insertQuery: function(baseQuery) {
    var pk = this.pkAttr;
    var dbattrs = this.pk ? this.dbattrs : _.omit(this.dbattrs, pk);
    ...
    var query = baseQuery.unbind().insert(dbattrs).returning(pk);
  ...
  _updateQuery: function(baseQuery) {
    var dbattrs = _.omit(this.dbattrs, this.pkAttr);
    return this._whereQuery(baseQuery).unbind().update(dbattrs);
  },
   ...
   * @type {Object}
   */
  dbattrs: property(function() {
    return _.clone(this._dbattrs);
  }),
