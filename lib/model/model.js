...
var Model = Class.extend(/** @lends Model# */ {
  ...
  idAttr: property({ property: 'pkAttr' }),
  ...
  save: BluebirdPromise.method(function(options) {
    ...
    else if (this._deleted === 'pending') {
      query = this.__identity__.objects
        .where(where).delete()
        .execute()
        .tap(function() {
          self._deleted = 'complete';
        });
    }
    else if (this.deleted) {
      // no action for object that was already deleted
    }
   ...
   * @type {Boolean}
   */
  deleted: property(function() { return this._deleted !== undefined; }),
