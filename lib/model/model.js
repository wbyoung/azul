...
var Model = Class.extend(/** @lends Model# */ {
  ...
  _insertQuery: function() {
    // note that `this.attrs` represents the database field names and that we
    // don't want to apply transformations to those names based on the model's
    // attributes. calling `insert([])` gets us an `InsertQuery` which is not
    // bound to this model & will not perform field name transformations. the
    // call to `values` below will set the values without transformations.
    var pk = this.pkAttr;
    var attrs = this.pk ? this.attrs : _.omit(this.attrs, pk);
    var query = this.__identity__.objects
      .insert([]).values(attrs).returning(pk);
  ...
  _updateQuery: function() {
    // note that `this.attrs` represents the database field names and that we
    // don't want to apply transformations to those names based on the model's
    // attributes. calling `update()` gets us an `UpdateQuery` which is not
    // bound to this model & will not perform field name transformations. the
    // call to `set` below will set the values without transformations.
    var attrs = _.omit(this.attrs, this.pkAttr);
    return this._whereQuery().update().set(attrs);
  },
