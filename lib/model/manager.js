...
var BoundQuery = require('../query/bound');
...
var Manager = Property.extend(/** @lends Manager# */ {
  ...
  _getter: function() {
    ...
    return function() {
      var modelClass = this;
      var load = modelClass.load.bind(modelClass);
      var transform = function(result, queryType) {
        var apply = _.any([SelectQuery, RawQuery, BoundQuery], function(type) {
          return queryType instanceof type.__metaclass__;
        });
        return apply ? result.rows.map(load) : result.rows;
      };

      var query = modelClass.query
        .bindModel(modelClass)
        .transform(transform);
