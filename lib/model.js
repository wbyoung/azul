...
// TODO: how to allow `user.articles.add` while not allowing `user.articles[0]`
// because the data hasn't been retrieved yet.
// would like to throw an error when trying to access `user.articles`, but not
// throw one when accessing `user.articles.add`.
//
// one idea is to use the following methods:
//
//   `user.addPost`
//   `user.addPosts`
//   `user.clearPosts`
//   `user.removePost`
//   `user.removePosts`
//   `user.createPost` (or just do this through addPost?)
//
// another idea:
// consider using harmony Proxy to enable error throwing for accessing anything
// within `user.articles` like `user.articles[0]` or `user.articles.length`
// when the objects have not been fetched yet.
 ...
 *
 *
 *     var article = Post.create({ title: 'Example' });
