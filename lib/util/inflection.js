...
Inflection.prototype.irregular = function(singular, plural) {
  var rule = function(type, transform, l0, lrest, r0, rrest) {
    var format = '\\b(%s)%s\\b';
    var source = util.format(format, l0[transform](), lrest);
    var regex = new RegExp(source);
    var replacement = r0[transform]() + rrest;
    this[type](regex, replacement);
  }.bind(this);
  ...
  rule('plural', 'toUpperCase', s0, srest, p0, prest);
  rule('plural', 'toLowerCase', s0, srest, p0, prest);
  rule('plural', 'toUpperCase', p0, prest, p0, prest);
  rule('plural', 'toLowerCase', p0, prest, p0, prest);

  rule('singular', 'toUpperCase', s0, srest, s0, srest);
  rule('singular', 'toLowerCase', s0, srest, s0, srest);
  rule('singular', 'toUpperCase', p0, prest, s0, srest);
  rule('singular', 'toLowerCase', p0, prest, s0, srest);
};
...
Inflection.prototype._regex = function(value) {
  ...
  var flags = isRegex ? (value.ignoreCase ? 'i' : '') : 'i'; // TODO: REGEX FLAGS
