...
SQLite3Adapter.reopenClass(/** @lends SQLite3Adapter */ {
  ...
  Procedures: Adapter.Procedures.extend({
    ...
    /**
     * Whether or not to use the `alterTable` procedure.
     *
     * @method
     * @public
     ...
     * @return {Boolean}
     */
    useAlterTable: function(data) {
      return data.dropped.length > 0;
    },
     ...
     * @return {Promise}
     */
    alterTable: function(baseQuery, data) {
      var table = data.name;
      var add = data.added;
      ...
      .then(function() { return query.raw('PRAGMA foreign_keys').fetchOne(); })
      .get('foreign_keys')
      .then(function(enabled) { this.fksEnabled = enabled; })
      .then(function() { return query.raw('PRAGMA foreign_keys=0'); })
      .then(function() {
      ...
      .then(function() {
        var definitions = this.columns.map(function(column) {
          ...
          var definition = fmt('%s %s', quote(column.name), column.type);
      ...
      .then(function() {
        var fields = _.map(this.columns, 'name').map(quote).join(', ');
        return query.raw(fmt('INSERT INTO %s (%s) SELECT %s FROM %s',
          quote(table), fields, fields, quote(table + '_old')));
      })
      .then(function() {
        return query.raw(fmt('DROP TABLE %s', quote(table + '_old')));
      })
      .then(function() { return query.raw('PRAGMA foreign_keys=' + this.fksEnabled); })
